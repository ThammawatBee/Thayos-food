...
export const generateHistoryParam = (search: LogSearch) => {
  ...
  return pickBy({ startDate, endDate, type: search.type }, (value) => !!value)
}

const useHistoryStore = create<LogState>()(
  devtools((set, get) => ({
    logs: null,
    count: 0,
    offset: 0,
    ...
    onPageSizeChange: async (pageSize: number) => {
      ...
      const response = await listLogs({
        limit: pageSize,
        offset: 0,
        ...generateHistoryParam(search) as any
      });
